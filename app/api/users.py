...
@bp.route('/concerts/<int:id>', methods=['GET'])
def get_concert(id):
    return db.get_or_404(Concert, id).to_dict()
...
def get_concerts():
    page = request.args.get('page', 1, type=int)
    per_page = min(request.args.get('per_page', 10, type=int), 100)
    return Concert.to_collection_dict(sa.select(Concert), page, per_page, 'api.get_concerts')
